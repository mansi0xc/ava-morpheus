...
// export async function postActivity(payload: ActivityPayload) {
...
// export async function getRecentActivities(walletAddress: string) {
//   const res = await fetch(`${API_BASE}/api/v1/activities/${walletAddress}`);
//   if (!res.ok) throw new Error(`getRecentActivities failed: ${res.status}`);
//   return res.json();
// }
