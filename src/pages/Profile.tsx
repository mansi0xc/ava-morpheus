...
import React, { useEffect, useState } from 'react';
import { User, Calendar, MapPin, Trophy, Settings } from 'lucide-react';
import { SteampunkNavbar } from '@/components/steampunk/SteampunkNavbar';
import { OrnateFrame } from '@/components/steampunk/OrnateFrame';
import {
  OrnateCard,
...
// NEW: API helpers
import { getRecentActivities, upsertProfile, type Activity } from '@/lib/api';
...
const Profile: React.FC = () => {
  ...
  const onEditOrSave = async () => {
    if (editMode) {
      ...
      // Also upsert to backend if wallet available
      if (walletAddress) {
        try {
          await upsertProfile({
            walletAddress,
  ...
  // ---------- Fetch activities for this wallet ----------
  useEffect(() => {
    if (!walletAddress) return;
    setLoadingAct(true);
    setActError(null);

    // Only show latest “Solved Clockwork Sudoku”
    getRecentActivities(walletAddress, {
      limit: 1,
      action: 'Solved',
      puzzle: 'Clockwork Sudoku',
    })
      .then(setActivities)
      .catch((e) => setActError(e.message ?? 'Failed to load activity'))
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <OrnateFrame>
              ...
              {loadingAct && (
                <div className="space-y-3">
                  {[...Array(2)].map((_, i) => (
                    <div
                      key={i}
                      className="h-16 animate-pulse rounded-ornate bg-surface-elevated/60 border border-primary/20"
                    />
                  ))}
                </div>
              )}
              ...
              {actError && (
                <div className="p-4 rounded-ornate border border-red-700/40 text-red-300 bg-red-900/20">
                  {actError}
                </div>
              )}

              {!loadingAct && !actError && activities.length === 0 && (
                <div className="p-4 rounded-ornate border border-primary/20 text-muted-foreground">
                  No activity yet for Clockwork Sudoku. Solve it to see it here!
                </div>
              )}
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {timeAgo(a.createdAt)} · {a.difficulty ?? '—'} · {formatDuration(a.timeTakenMillis)}
                          </p>
                        </div>
                      </div>
                      <div className="text-primary font-ornate font-semibold text-sm">
